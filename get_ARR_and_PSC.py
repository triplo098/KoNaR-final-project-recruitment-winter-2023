### DISCLAIMER: IT'S NOT MY CODE, IT'S from https://stackoverflow.com/a/53548161
import numpy as np
import pandas as pd

TARGET_F = 26  # In Hz so 50.0 is 0.020 seconds period and 0.25 is 4 seconds period
CLOCK_MCU = 80000000
TOLERANCE = 0.00001
...
for index in range(len(exact_prescalers)):
    ...
    for rowindex in range(len(rows)):
        df = df._append(pd.DataFrame(np.array(rows[rowindex]).reshape(1, 4), columns=df.columns))
...
df = df._append(pd.DataFrame(np.array(close_prescalers).reshape(len(close_prescalers), 4), columns=df.columns))
